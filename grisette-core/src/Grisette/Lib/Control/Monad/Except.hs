...
mrgThrowError :: (SymBoolOp bool, MonadError e m, MonadUnion bool m, GMergeable bool a) => e -> m a
mrgThrowError = merge . throwError
{-# INLINE mrgThrowError #-}

-- | 'catchError' with 'Mergeable' knowledge propagation.
mrgCatchError ::
  (SymBoolOp bool, MonadError e m, MonadUnion bool m, GMergeable bool a) =>
  m a ->
  (e -> m a) ->
  m a
