...
doPevalFiniteBitsSymShiftShiftLeftTerm ::
  ...
  Term a ->
  Maybe (Term a)
doPevalFiniteBitsSymShiftShiftLeftTerm (ConTerm _ _ a) (ConTerm _ _ n)
  | n >= 0 =
      if (fromIntegral n :: Integer) >= fromIntegral (finiteBitSize n)
        then Just $ conTerm zeroBits
        else Just $ conTerm $ symShift a n
doPevalFiniteBitsSymShiftShiftLeftTerm x (ConTerm _ _ 0) = Just x
-- TODO: Need to handle the overflow case.
-- doPevalShiftLeftTerm (ShiftLeftTerm _ x (ConTerm _ _ n)) (ConTerm _ _ n1)
--   | n >= 0 && n1 >= 0 = Just $ pevalShiftLeftTerm x (conTerm $ n + n1)
doPevalFiniteBitsSymShiftShiftLeftTerm _ (ConTerm _ _ n)
...
doPevalFiniteBitsSymShiftShiftRightTerm ::
  ...
  Term a ->
  Maybe (Term a)
doPevalFiniteBitsSymShiftShiftRightTerm (ConTerm _ _ a) (ConTerm _ _ n)
  | n >= 0 && not (isSigned a) =
      if (fromIntegral n :: Integer) >= fromIntegral (finiteBitSize n)
        then Just $ conTerm zeroBits
...
doPevalFiniteBitsSymShiftShiftRightTerm x (ConTerm _ _ 0) = Just x
-- doPevalFiniteBitsSymShiftShiftRightTerm (ShiftRightTerm _ x (ConTerm _ _ n)) (ConTerm _ _ n1)
--   | n >= 0 && n1 >= 0 = Just $ pevalFiniteBitsSymShiftShiftRightTerm x (conTerm $ n + n1)
doPevalFiniteBitsSymShiftShiftRightTerm _ (ConTerm _ _ n)
  | not (isSigned n)
