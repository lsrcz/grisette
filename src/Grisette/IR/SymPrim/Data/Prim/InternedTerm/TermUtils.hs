...
identityWithTypeRep (TestBitTerm i _ _) = (typeRep (Proxy @t), i)
...
introSupportedPrimConstraint TestBitTerm {} x = x
...
extractSymbolicsSomeTerm t1 = evalState (gocached t1) M.empty
  where
    ...
    go (SomeTerm (TestBitTerm _ arg _)) = goUnary arg
...
pformat (TestBitTerm _ arg n) = "(testbit " ++ pformat arg ++ " " ++ show n ++ ")"
...
someTermsSize terms = S.size $ execState (traverse goSome terms) S.empty
  where
    ...
    go t@(TestBitTerm _ arg _) = goUnary t arg
